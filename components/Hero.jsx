    ...
    <section className="relative min-h-screen flex items-center hero-pattern overflow-hidden pt-20">
      ...
      <div className="absolute top-0 right-0 w-1/2 h-full opacity-5 pointer-events-none">
        <svg viewBox="0 0 400 400" className="w-full h-full">
          <defs><pattern id="rp1" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
            <polygon points="20,0 40,20 20,40 0,20" fill="none" stroke="#B45309" strokeWidth="1"/>
          </pattern></defs>
          <rect width="400" height="400" fill="url(#rp1)"/>
        </svg>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-20 grid md:grid-cols-2 gap-12 items-center w-full">
        <div className="space-y-6">
          <div className="inline-flex items-center gap-2 bg-amber-100 border border-amber-200 text-amber-800 text-sm font-medium px-4 py-1.5 rounded-full animate-fade-in">
            ...
            Melayani Area Klaten & Sekitarnya
          </div>
          <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-stone-900 leading-tight animate-fade-up">
            Rumah Sejuk,{' '}
            <span className="text-amber-700">Tagihan Hemat</span>{' '}
            Sepanjang Tahun
          </h1>
          <p className="text-stone-600 text-lg leading-relaxed animate-fade-up delay-100">
            Insulasi atap berkualitas tinggi untuk kenyamanan maksimal. Kurangi panas terik, hemat listrik AC, dan buat rumah Anda lebih tenang — mulai dari harga terjangkau.
          </p>
          <div className="flex flex-wrap gap-2 animate-fade-up delay-200">
            {['✓ Gratis Konsultasi', '✓ Harga Kompetitif', '✓ Pengiriman ke Lokasi', '✓ Terpercaya'].map(item => (
              <span key={item} className="text-sm bg-stone-100 text-stone-700 px-3 py-1 rounded-full font-medium">{item}</span>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-3 pt-2 animate-fade-up delay-300">
            ...
            <a href="#produk" className="flex items-center justify-center gap-2 bg-white border-2 border-amber-700 text-amber-800 font-bold px-6 py-4 rounded-2xl hover:bg-amber-50 transition-all text-base">
              Lihat Produk <ChevronDown />
            </a>
          </div>
          <div className="flex gap-8 pt-4 border-t border-stone-200 animate-fade-up delay-400">
            {[{ num: '500+', label: 'Proyek Selesai' }, { num: '5 Thn', label: 'Pengalaman' }, { num: '4.9★', label: 'Rating Pelanggan' }].map(s => (
              <div key={s.label}>
                <div className="text-2xl font-display font-bold text-amber-700">{s.num}</div>
        ...
        <div className="relative animate-fade-up delay-200">
          <div className="bg-gradient-to-br from-amber-800 to-amber-900 rounded-3xl p-8 text-white shadow-2xl shadow-amber-900/30 roof-texture">
            ...
            <h3 className="font-display text-2xl font-bold mb-2">Insulasi Atap Klaten</h3>
            <p className="text-amber-200 text-sm mb-6">Solusi peredam panas & suara terbaik</p>
          ...
          <div className="absolute -top-4 -right-4 bg-white rounded-2xl shadow-xl p-3 border border-amber-100 text-center">
          ...
          <div className="absolute -bottom-4 -left-4 bg-white rounded-2xl shadow-xl px-4 py-2 border border-amber-100 flex items-center gap-2">
            <span className="text-lg">📍</span>
            <div>
              <div className="text-xs font-bold text-stone-800">Melayani</div>
              <div className="text-xs text-amber-600 font-semibold">Klaten & Sekitarnya</div>
            </div>
          </div>
        </div>
      </div>
