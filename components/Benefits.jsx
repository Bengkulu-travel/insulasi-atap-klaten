...
export default function Benefits() {
  const allServiceAreas = storeInfo.branches
    ? storeInfo.branches.flatMap(b => b.serviceArea || []).filter((v, i, a) => a.indexOf(v) === i)
    : []
  ...
  const reasons = [
    ...
    { icon: "🚚", title: "Pengiriman ke Lokasi", desc: "Layanan antar langsung ke lokasi proyek Anda di Klaten, Yogyakarta, dan sekitarnya." },
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-14">
          ...
          <p className="text-stone-600 max-w-xl mx-auto">
            Kami bukan sekadar penjual. Kami adalah mitra Anda dalam menciptakan hunian yang nyaman dan hemat energi.
          </p>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {reasons.map((r) => (
            <div key={r.title} className="group p-6 rounded-2xl border border-stone-100 hover:border-amber-200 hover:bg-amber-50/30 transition-all duration-300">
              <div className="text-3xl mb-3">{r.icon}</div>
              <h3 className="font-display text-lg font-bold text-stone-900 mb-2 group-hover:text-amber-800 transition-colors">{r.title}</h3>
        ...
        <div className="bg-gradient-to-r from-amber-800 to-amber-900 rounded-3xl p-8 md:p-10 text-white roof-texture">
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
            <div className="flex-1">
              <h3 className="font-display text-2xl md:text-3xl font-bold mb-2">Area Layanan Kami</h3>
              <p className="text-amber-200 text-sm mb-4">Klaten & Yogyakarta — pengiriman langsung ke lokasi Anda</p>
