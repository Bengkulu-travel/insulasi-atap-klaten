import { storeInfo } from '../app/data'

export default function Location() {
  const waUrl = `https://wa.me/${storeInfo.whatsappNumber}?text=${encodeURIComponent('Halo, saya ingin tahu lokasi toko insulasi atap Klaten. Bisa kirim alamat lengkap?')}`

  return (
    <section id="lokasi" className="py-20 bg-white">
      <div className="max-w-6xl mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-14">
          <div className="inline-flex items-center gap-2 bg-green-50 text-green-800 text-sm font-semibold px-4 py-1.5 rounded-full mb-4">
            📍 Temukan Kami
          </div>
          <h2 className="font-display text-4xl md:text-5xl font-bold text-stone-900 mb-4">
            Lokasi & Kontak
          </h2>
          <div className="section-divider mx-auto mb-4"></div>
        </div>

        <div className="grid md:grid-cols-2 gap-8 items-start">
          {/* Map */}
          <div className="rounded-3xl overflow-hidden shadow-lg border border-stone-100 h-80 bg-stone-100 flex flex-col items-center justify-center gap-4">
            {/* Embed Google Maps */}
            <iframe
              src="https://maps.google.com/maps?q=Klaten,+Jawa+Tengah,+Indonesia&output=embed&z=13"
              width="100%"
              height="100%"
              style={{ border: 0, minHeight: '320px' }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              className="w-full h-full"
            />
          </div>

          {/* Contact Info */}
          <div className="space-y-4">
            {/* Address Card */}
            <div className="p-5 bg-amber-50 rounded-2xl border border-amber-100 flex gap-4">
              <div className="text-2xl flex-shrink-0">📍</div>
              <div>
                <div className="font-bold text-stone-900 mb-1">Lokasi</div>
                <div className="text-stone-600 text-sm">{storeInfo.address}</div>
                <a 
                  href={storeInfo.googleMapsUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-amber-700 text-sm font-semibold hover:underline mt-1 inline-block"
                >
                  Buka di Google Maps →
                </a>
              </div>
            </div>

            {/* Phone Card */}
            <div className="p-5 bg-green-50 rounded-2xl border border-green-100 flex gap-4">
              <div className="text-2xl flex-shrink-0">📱</div>
              <div>
                <div className="font-bold text-stone-900 mb-1">WhatsApp / Telepon</div>
                <div className="text-stone-600 text-sm">{storeInfo.phone}</div>
                <a
                  href={waUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-green-600 text-sm font-semibold hover:underline mt-1 inline-block"
                >
                  Chat Sekarang →
                </a>
              </div>
            </div>

            {/* Hours Card */}
            <div className="p-5 bg-blue-50 rounded-2xl border border-blue-100 flex gap-4">
              <div className="text-2xl flex-shrink-0">🕐</div>
              <div>
                <div className="font-bold text-stone-900 mb-1">Jam Operasional</div>
                <div className="text-stone-600 text-sm">{storeInfo.openHours}</div>
                <div className="text-stone-400 text-xs mt-1">Minggu & hari besar: tutup</div>
              </div>
            </div>

            {/* WA Button */}
            <a
              href={waUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-3 w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-2xl transition-all hover:shadow-xl hover:shadow-green-500/30 text-base"
            >
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              Hubungi via WhatsApp — {storeInfo.phone}
            </a>
          </div>
        </div>
      </div>
    </section>
  )
}
